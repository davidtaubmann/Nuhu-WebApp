...
function cardBase(card,vals,USR){
	...
	return new Promise(function(resolve,reject){
		...
		CB += "		<\/div>";
		CB += "		<input class=\"NUHU-card-actFase2 not-sync\" type=\"checkbox\" id=\"C"+x+"-AF2\" name=\"actFase2-"+x+"\" value=\"C"+x+"\" OLDonChange=\"marcAcc("+x+",'"+card.id+"'); \" \/>";
		...
		/*/
		if(!card.admin)CB += "			<select name=\""+card.id+"\" id=\"CanNmr-"+x+"\" class=\"NMR mdl-color--primary mdl-color-text--primary-contrast mdl-button--icon \" onChange=\"canChng('"+categ+"','"+card.id+"','"+x+"',this.options[this.selectedIndex].value);\" title=\"Agregar "+card.id+" a tu canasta\" /><option value selected disabled>+</option><option value="+Number(vals.und)+">"+Number(vals.und)+"</option><option value="+Number(vals.und)*10*2/10+">"+Number(vals.und)*10*2/10+"</option><option value="+Number(vals.und)*10*3/10+">"+Number(vals.und)*10*3/10+"</option><option value="+Number(vals.und)*10*4/10+">"+Number(vals.und)*10*4/10+"</option><option value="+Number(vals.und)*10*5/10+">"+Number(vals.und)*10*5/10+"</option><option value="+Number(vals.und)*10*6/10+">"+Number(vals.und)*10*6/10+"</option><option value="+Number(vals.und)*10*7/10+">"+Number(vals.und)*10*7/10+"</option><option value="+Number(vals.und)*10*8/10+">"+Number(vals.und)*10*8/10+"</option><option value="+Number(vals.und)*10*9/10+">"+Number(vals.und)*10*9/10+"</option></select><br />";
		/**/
		if(!card.admin)CB += "			<button id=\"CanRem-"+x+"\" class=\"mdl-color--accent mdl-color-text--accent-contrast mdl-button mdl-button--icon mdl-js-button mdl-js-ripple-effect oculto\" title=\"Quitar "+card.id+" de tu canasta\" onclick=\"canRemProd('"+categ+"','"+card.id+"','"+x+"');\"> <i class=\"material-icons\">clear<\/i> <\/button>";
